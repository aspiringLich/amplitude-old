<script lang="ts">
    import { File, InfoCircled, ExclamationTriangle, CheckCircled, CrossCircled, Cross2 } from "radix-icons-svelte";
    import colors from "tailwindcss/colors";

    // Props
    /** The type of the admonition. Only accepts certain strings. */
    export let type: "note" | "info" | "warning" | "success" | "correct" | "failiure" | "incorrect" | "error";
...
<div class="admonition my-4 flex flex-col {classes}">
    <div class="title rounded-t w-full text-white font-extrabold flex items-center" style:background-color={colors[color_type][500] + "80"}>
        <svelte:component this={icon} size={18} class="m-1.5" />
        <span>{title_text}</span>
    </div>
    <blockquote class="rounded-t-none {container}" style:background-color={colors[color_type][300] + "80"}>
        <slot />
    </blockquote>
</div>
